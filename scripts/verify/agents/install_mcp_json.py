"""Verification: the hub writes an agent's .mcp.json into its workdir (design §8/D8)
plus the agent-side profile .claude/settings.local.json (D21).
...
# A git checkout (the token-carrying names go into its .gitignore) that
...
print(f"gitignore  : {result['gitignore']}   <- added to the checkout's .gitignore")
print("  " + (workdir / ".gitignore").read_text().replace("\n", "\n  ").rstrip())

hr("2. SETTINGS  (the agent-side profile)")
...
print(f"model      : {sdoc['model']}   <- as declared at registration")
print(f"status line: {sdoc['statusLine']['command']}   <- names the agent's terminal")
