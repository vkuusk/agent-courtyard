"""Verification: the channel-flag report and the delivery-verification check
(items 33/34, D29/D30).
...
Needs the compose postgres up (`make db-up`). Run:
    uv run python scripts/verify/messages/delivery_check.py
"""
...
load_env_file()  # the compose project and port this checkout uses
...
try:
    ...
    team_dir = tempfile.mkdtemp(prefix="verify-team-")
    admin.set_current_team(admin.add_team(team_dir, "verify").id)
    hr("1. THE CHANNEL FLAG: attach reports how the session was launched")
    ...
    hr("2. THE DELIVERY CHECK: what the model receives, and the ack")
