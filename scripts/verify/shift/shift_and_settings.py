"""Verification: the shift state machine and the Team settings (design §8.1, D23).

  1. settings: defaults, terminal app round trip, `always_on` and junk values refused
  2. shift: off -> starting (the verification countdown ALWAYS runs — D28:
...
admin.register_agent(name, "dummy", "verify twin", None, None)
...
# custom terminal apps: add, select, and the refusals; restored afterwards
saved = admin._call(
    "PATCH",
    "/api/settings",
