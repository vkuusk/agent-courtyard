"""Verification: a terminal spawner opens a window, proves it alive, and closes it
...
Run it once per terminal application you support:
    ...
    uv run python scripts/verify/shift/terminal_spawners.py            # Ghostty
    uv run python scripts/verify/shift/terminal_spawners.py Terminal
    uv run python scripts/verify/shift/terminal_spawners.py iTerm2
...
MARKER = f"courtyard-verify-{int(time.time())}"
WORKDIR = "/tmp"
