...
- **Date:** 2026-08-18
- **Status:** living design document; every decision and its reasons are in the log (§13)
- **Authors:** vkuusk, with Claude
...
- No embedding/RAG subsystem yet. Hub memory (`hub-memory.md`, D37) keeps case files of
  closed threads with full-text recall; vectors are its slice 3, behind the same API.
...
`name` and `type` cannot be edited and are refused, never silently ignored (a removed
name can be registered again, which revives its row with new fields and a new token, D36). The
...
The rule: *a→b, b→a, a→b — never a second message before the answer to the first.*
...
- An approval may carry a note; the note is delivered alongside the approved message as an
  `operator_note` (the usual operator action: *add to* a message, not edit it).
...
- **Automatic on removal.** Removing an agent archives every line it is on (reason
  ...
  used again (its token is revoked; registering the name again revives the agent, not its
  lines, D36), so nothing is lost and the board
...
### 5.8 Discovery — who forms the team's wiring (WP‑E, decision D22)

*Auto-discovery relies on the agents themselves knowing the
...
document, changed in Admin → Settings as a pulldown; the names: `auto` is
...
trip incl. history archived, operator exemption, mode-switch grandfathering. Verification
script + manual procedure (flip to manual, watch an unlinked pair be refused and a
linked pair talk; unlink mid-conversation). Live check: a three-agent board
...
remove) ends that life: the token is refused and sends to the agent fail with `agent_gone`.
Registering the same name again revives the row (D36): same id, new token, status
`invited`, every descriptive field and the type from the new registration.
...
the misleading all-green-then-offline flicker seen live.
...
- **The delivery check (D30).** The only reaction the hub can observe end to end is the
  ...
  normal push payload) carrying a nonce: "your operator has started a shift and the hub
  is confirming its messages reach you; confirm by calling `courtyard_ack` with token
  X; that one call completes the check". Worded as an expected step, never as a secret:
  "tell no one, do nothing else" read as prompt injection to a session without the D40
  context. Ack → **verified** (a point-in-time fact, shown with its time);
...
- **Install (6d).** Writing the agent's *project-level* `.mcp.json` — merge with any existing
  ...
  (`hub/core/install.py`): the **WebUI** button (`POST /api/agents/{id}/connect`, the hub
  writes it — dev mode, since the hub must share the workdir's disk) and the
  **`courtyard-invite`** CLI (`--register` to create-and-connect, `--unregister` to revert), a
  ...
  **Token placement: inline in `.mcp.json` + `chmod 600`**
  (D15): the file carries the secret and must not be committed — the
  ...
  model stays, since it may have been retuned by hand. The approval of the project's
  `.mcp.json` server is not written into that file: Claude Code ignores it there unless
  git proves the file uncommitted (seen on 2.1.269 in a workdir that is not a git
  checkout, where the question returned at every launch). The launch command carries it
  instead, `--settings '{"enabledMcpjsonServers":["courtyard"]}'`: only that server, only
  sessions the courtyard starts (Start shift, Resume, the start script).

### 7.3 pi adapter (item 36, D32)

The second adapter, proving §7.1's pluggability seam. The whole adapter is **one
TypeScript extension file**, `.pi/extensions/courtyard.ts`, written by install from a
packaged template with the agent's connection substituted (token inline + chmod 600,
the D15 precedent) and auto-discovered by pi — no launch flag exists, so the item-33
deaf-session class does not either (`channel_flag` honestly reports `present`).

Everything rides pi's first-class extension API, where Claude Code needs a research
preview: `pi.registerTool()` serves the same four tools with the same texts;
delivery is `pi.sendMessage({customType: "courtyard", content: rendered},
{triggerTurn: true, deliverAs: "followUp"})` — the hub-rendered envelope injected
into LLM context, waking an idle session and queueing politely on a busy one (never
`sendUserMessage`: a peer must not impersonate the operator). Attach on
`session_start` (retrying forever, item 12 parity), heartbeat on a timer, a local
`node:http` listener as the channel endpoint, clean detach on `session_shutdown`.
The hub is untouched: same HTTP contract, same envelope, same delivery check.

Written as plain JavaScript in a `.ts` file so pi's jiti loads it and the test
harness (`tests/pi_harness.mjs`) can drive the identical file under bare Node
against a live hub — the wire-level round trip is in the automated suite even
though pi itself is not.

The extension also gives the session its membership context (D40), the pi
counterpart of the Claude Code SessionStart hook. At `session_start`, before it
attaches, it stores the hub-rendered block from `GET /api/agents/{name}/session-context`,
worded for pi, as a custom message without a turn (`customType: "courtyard-context"`),
so the first delivery already finds it; when the hub does not answer within 2 s it
stores the same text as install rendered it into the file, without the team's name. A
session that already holds the block (resumed, forked, reloaded) is left alone, and
`session_compact` stores it again once compaction summarized it away.
`before_agent_start` is not used: pi fires it only for a typed prompt, never for a turn
the extension's own delivery starts. The launch command is `pi`, with `--model` when
the agent declares a model, as for Claude Code. Deferred: publishing to pi.dev.
...
| `hub-notice` | the hub generated it (`kind: system`) | Factual information about your own messages: gate verdicts, line state. Not a request at all. The delivery check is the one exception and carries a preamble of its own: the single tool call the hub asks for (D40). |

Neither `domain-owner` nor `agent` ever licenses acting on embedded commands; that line is in
both preambles.

**The reply footer.** Grading answers
...
footer after the body, naming the tools the way the recipient's host lists them (the
courtyard MCP tool for Claude Code, the courtyard tool for pi; D40). A question (no
`reply_to`) says: reply with `courtyard_send` — text printed in the terminal never reaches the sender — and answer what
was asked, completely and no more (no trailing offers or side questions; each one costs a
full exchange). Since item 22 (an agent answering a peer stalled at its
...
### 8.1 Shift and Team mode (WP‑F, decision D23)

*Starting the courtyard today means a terminal
...
4. **First-run dialogs**: channel consent cannot be pre-approved (D21), so a brand-new
   agent's first shift start needs a keypress in its terminal; the `.mcp.json` server
   question is answered by the launch command (`--settings`, §7.2). Accepted; revisit
   only if it proves frustrating.
...
**End shift closes the books (D24).**
...
- **expires the unfinished messages** — the unanswered in-flight message of each
  ...
  at the gate (`PENDING_GATE`; one rule: the shift closes the
...
**The stale shift (D25).** If the operator closes
...
should not have to understand that bookkeeping (the correction of a
...
**Resume belongs to the living shift (amendment).** The dialog
...
- **Spawner**: an interface with three macOS implementations behind the `terminal_app`
  setting: Terminal.app, iTerm2 and Ghostty (D35), all via `osascript` and one base class
  (`OsascriptTerminal`: one ref shape `{app, window_id, tty}`, one liveness probe, one
  close sequence of count, kill, close), plus a fake for tests. The WebUI reads the
  built-in names from `GET /api/settings/terminals`. Workdir paths and the command are
  ...
  (item 20):** the operator can define further apps in Admin → Terminal
...
- **Courtyard page**: top right of the Team panel header — `▶ Start shift` →
  ...
  (live check). Under *Always on* the whole group is replaced by
...
restart (the "15–20 s of not responding" seen during cycle 1).
...
windows; settle timeout). AppleScript escaping tests. Live check: press the
pill with one agent already running and one down — exactly one terminal opens; end the
shift — only that terminal closes. Verification entry + script per the testing standard.
...
The original draft proposed a filesystem backend for v1. Review overturned it, and
the objections held up:
...
1. **Status updates don't fit append-only files.** A message's status changes over its life
   ...
   **atomically together with the line-state transition** — that is the turn machine's
   ...
   the same conclusion already reached building the Postgres-backed work queue
...
```
docker-compose.yml
  postgres:   always on   (pgvector/pgvector:pg18, named volume, host port 26432)
...
operator's terminal windows (§8) and where `make run` and the installed hub are the
same program with the same `.env`. Both bind `127.0.0.1` only. One machine, one
...
in the middle — no title strip — and **one input box pinned to the bottom of the Courtyard page** (amended from "every page" by item 20: the box felt out of place on the admin-ish pages — Agents, Archive and Admin have none; all typing happens on the Courtyard page). The operator always types in the same place; *what* the text
...
├── docs/
│   └── design/                     # this doc and the companion design documents
├── src/courtyard/
...
|---|---|---|---|
| D1 | Hub-and-spoke exchange board; agents never talk directly | **Accepted** | The core idea |
| D2 | Strict turn-taking: one unanswered message in flight per line | **Accepted** | §5.4 formalization — confirm the "either side may initiate from IDLE" reading |
...
| D10 | Approver is a pluggable interface; human-only implementation in v1 | **Accepted** | §5.5 |
| D11 | One Python package, multiple entry points; adapters installed by config, zero-fork | **Accepted** | §12, §7.2 |
| D12 | Deployment = docker compose for postgres, the hub on the host: dev_mode (`make run`) and the installed mode (a LaunchAgent, D38) | **Accepted** | §9.4; the hub container once planned here was dropped by D38 |
| D13 | Migrations: forward-only numbered plain-SQL files + the ~40-line custom runner (startup-applied, one tx per file); no migration tool, no down-migrations in v1. Recovery = new forward migration; dev data is disposable (`make db-nuke`), precious data gets `pg_dump` first | **Accepted** | Reviewed Flyway/Liquibase/Prisma/Alembic/yoyo/pgroll — all re-buy what we have at this scale. Revisit triggers: a second deployed environment; parallel dev colliding on numbers (→ yoyo); zero-downtime v2 service (→ pgroll). Format imports into Flyway/yoyo nearly as-is, so no lock-in |
| D-spike | Claude adapter delivery stack (spike 6a, verified on Claude Code 2.1.237): **(1) channels** — MCP stdio server with the `claude/channel` experimental capability pushes `notifications/claude/channel` events that arrive as live turns; primary mechanism for open sessions (events queue while busy per docs). **(2) Stop hook** — backstop at end-of-turn; emits both `systemMessage` and `reason`; loop-guarded by `stop_hook_active` + Claude Code's 8-consecutive-block override; unread state queried from the **hub API only**, no local mailbox/state files. **(3) `claude -p --resume <name>`** — context-preserving delivery/wake for **closed** sessions only: delivering into an open session forks the transcript tree and orphans the delivered branch (verified empirically). Adapter = one stdio MCP server per agent (channels are stdio-only), exposing the courtyard tools on the same server | **Verified by spike** (all three experiments run) | Operational note: while channels are in research preview, launch commands need `--dangerously-load-development-channels server:courtyard` and a per-start consent screen. The contract **drifted twice in four days**: 2.1.241 stopped honouring the flag ("not in --channels list"), 2.1.245 restored it — and made the two-flag workaround (`--channels` + the dev flag) fail the allowlist check. A wrongly-launched session still connects, serves tools and ACKs deliveries while silently dropping them; the tell is "Channel notifications skipped" vs "registered" in the courtyard MCP log (`~/Library/Caches/claude-cli-nodejs/<project>/mcp-logs-courtyard/`). `tests/communications/oper-agent1-oper.py` proves the live round trip against a real session — run it after any Claude Code update before blaming the hub. Bonus finding: the delivered-content-is-data framing (now graded, §7.5) held at the `instructions` level (agent refused a redirect attempt). **Adoption (D14):** (1) primary; (3) reserved as an operator action; (2) verified but not installed in v1 |
| D16 | **v1 scope cut around the quickstart** — launch L1, live mode (6e/6f) and the pi adapter leave v1; v1 is Claude Code only, hub on the host. The freed room becomes step 7: the WebUI consolidated for the quickstart configuration (operator + two claude-code agents) — MainBoard absorbs the Gate and Inbox pages, Agent Admin stays, **Courtyard Admin** is added (housekeeping: dead registrations/lines; defaults: supervision mode; health). v1 acceptance = the quickstart scenario run through this UI. Step 7 is worked **agile, per page** (design proposal → review → implement → approve), not waterfall | **Accepted** | Functional, minimalistic, intuitive is the bar. §10 is updated per page as each is approved rather than re-specified up front |
| D15 | **Agent token placement: inline in `.mcp.json` + `chmod 600`.** The install writes the bearer token straight into the file's `env` block and locks the file 0600 | **Accepted** | Chosen over (b) `${COURTYARD_TOKEN}` expansion and (c) a separate 600 token file. Decider at the time: the hub revealed the plaintext token only once and never stored it, so (b) — the only option that keeps the secret out of a project file entirely — would force the operator to re-supply the token on every launch. D19 later made the hub keep tokens; inline placement stays, now for self-containedness across restarts alone. Inline is self-contained across restarts; the accepted cost is that `.mcp.json` (designed to be committable) now carries a secret, mitigated by 0600 + a "do not commit" warning in the install result rather than by editing the operator's `.gitignore` (which could un-track other MCP servers). Revisit if the hub ever persists reusable tokens, or agents span machines (→ token file or short-lived tokens) |
| D14 | **Minimal agent-side footprint — one MCP server + the launch flag + one SessionStart hook (D40), nothing else; no Stop hook in v1.** Hooks are not forbidden; each one is justified like any other item registration writes (`team-charter.md` §3). Whatever can live in the hub does: the authority envelope is rendered hub-side (`Message.rendered`), peer discovery is ranked/trimmed/worded hub-side (`GET /peers`), and blue-moon delivery failures are shown to the operator on the board instead of being patched agent-side. Hub→agent delivery = channel push to open sessions + backlog on attach + heartbeat-driven pull; a closed session's mail waits (durably `queued`) until the operator reopens its terminal — resume-wake is an operator action, never a hub reflex | **Accepted** | Every agent-side mechanism is one more thing to port per agent type. The Stop hook's unique coverage (adapter crashed while the session lives; channel silently broken) is hub-detectable — stale channel, line stuck in `awaiting_reply` — and operator-fixable; it cannot confirm model-read any better than the channel; and it is reversible at zero hub cost (`GET /inbox` already takes-and-marks). **Open fact, on the v1 acceptance checklist:** a channel event queued while the agent is busy must start a turn by itself when the turn ends (docs say so; spike A never exercised it). Turn-taking is per line, so busy-arrival is routine — fan-in from other lines, the operator's own terminal tasks — and if the fact fails, the hook is the only fix. Automatic `-p --resume` rejected for now: without a clean detach the hub cannot tell "closed" from "adapter crashed, session open" (and the latter forks — spike C); headless turns cannot be prompted for tool permissions; it is a host-side spawn that 6f's container cannot do |
| D17 | **The quickstart is a permanent product feature and the acceptance gate.** The quickstart — easy install/start plus one worked example, written up in `docs/quickstart.md` — is the convenience path a new operator follows to start using the courtyard for day-to-day work. It is not a demo and not merely the v1 acceptance scenario. v1 acceptance = running the quickstart as a new operator would and approving; each step-7 page is likewise approved after trying it in the browser (agile: proposal → review → implement → approve) | **Accepted** | Corrects the D16-era framing in which the quickstart existed for acceptance. Consequences: `docs/quickstart.md` is a v1 deliverable maintained alongside the pages rather than written once at the end (goal 8, §2), and the WebUI is judged by whether a new operator gets through it unaided (§10) |
| D18 | **WebUI rendered with Preact + htm, vendored, still no build step; and the approved layout** — collapsible side bar, agent rectangles, lines as two nodes + one colour-coded wire, a conversation pane showing whatever is selected, one input box at the bottom of every page (§10; narrowed to the Courtyard page only by item 20) | **Accepted** | The pain in the vanilla UI was keeping the screen in sync by hand (re-render races, preserved-input hacks), not styling; Preact + htm removes that at the cost of one 13 KB file and no toolchain, keeping D4's spirit. Alternatives weighed: Vue ESM (same benefit, HTML-looking templates, 10× the file), React/Svelte + Vite + Tailwind + shadcn (ready-made blocks, but node + npm + a build step and code the operator cannot read — the sprawl D4 fenced off). Layout choices confirmed live: the pane shows what you clicked; the input box is always in the same place, gate comments included; operator lines are not wires |
| D19 | **The hub keeps every agent's token; the operator can read an agent's launch config again and rotate its token.** Migration 0006 adds `agents.token` (plaintext) beside the hash; `GET /api/agents/{id}/token`, `POST /api/agents/{id}/token` (rotate: old token refused at once, channel dropped, agent reads as offline until restarted), connect and `courtyard-invite` no longer need a token passed in; the Agents page row has **launch config** (read-only, the files with the token) and **edit** holds **rotate token**, which rewrites the files at once | **Accepted** | "Personal-use app, definitely not a public SaaS" — the once-only token was a SaaS reflex that cost the operator the ability to re-open a config. Within the D3 threat model (one operator, one machine) a plaintext token in the local database adds nothing an attacker on the machine did not already have. Registrations from before 0006 have no stored token until rotated; the UI says so |
| D20 | **Archive: a line's history becomes one immutable JSON document in `lines_archive`; automatic when a participant is removed (and the line row goes), on request via an Archive button; an Archive page to re-read and export** (§5.7) | **Accepted** (all three points confirmed) | The ask: inactive lines dilute the board; keep history as a reminder and for offline audit in a dedicated table that can be backed up and cleaned up. Decisions to confirm: (1) one JSON document per archive rather than mirrored line/message tables; (2) removal deletes the archived line rows, and lines of already-removed agents are archived at the next hub start; (3) archiving a non-idle line releases it and archives the in-flight message as it stands, after a confirm that says so |
| D24 | **End shift closes the books; incidents re-deliver (§8.1, §5.4 rule 7, §6.4)** — ending the shift releases every non-idle line and marks unfinished messages `expired` (the unanswered in-flight message of each awaiting line **and** gate-held messages; kept in history with a `system` entry, nothing deleted); on attach, a `delivered`-but-unanswered message acknowledged before the current attachment flips back to `queued` and is re-pushed with a "redelivered" note (R1); the board shows "owes you a reply" on the agent card + real line state in the pane header (R3) | **Accepted** | The shift boundary (D23) is what item 10 lacked: normal path = the operator's explicit close-out, incident path (crash, restart mid-shift, out-of-shift agents, future `Always on`) = R1+R3. `expired` is what keeps R1 from resurrecting intentionally closed messages. Expire-not-delete follows D20's history-keeping philosophy; gate-held expiry follows one rule — the shift closes the working period. R2 (operator supersede) dropped: little value once end-shift cleans up routinely, and it would bend §5.4 asymmetrically |
| D22 | **Discovery: `auto` \| `manual` (§5.8)** — a courtyard-wide setting for who forms the team's wiring: `auto` (today) = every agent sees every other, lines form on first message; `manual` = agents see and can message only the agents the operator has **linked** — a link IS a pre-created idle line (no new tables; the line's existence is the permission), unlinked sends are refused with `not_linked`, peers/roster filter by line, **unlink** archives the history and removes the line; the operator is exempt (always reachable, no links needed); switching modes migrates nothing (existing lines become the links) | **Accepted** (`auto` from service-discovery vocabulary, `manual` = what the operator's hands do; "Team mode" rejected as the setting name — already taken by D23's lifecycle axis; operator exemption, grandfathering and unlink-archives-not-deletes confirmed) | The boundary moves into the hub — auto-discovery relied on agents knowing the interaction rules; manual makes sub-teams possible. Designed and implemented the same day (migration 0013: archive reason `unlinked`) |
| D26 | **A restarted hub verifies before it claims (§6.3)** — at startup, stored `connected`/`stale` agents flip to **`unknown`** (migration 0012); the sweep judges them only once the hub is a heartbeat interval + 5 s old (fast sweep cadence while judging), a heartbeat flips one straight to connected at any moment; UI: gray "checking…" dots, dimmed Team panel, `Checking the team · N` pill (`ShiftStatus.checking_until`); D25's question waits for every status to be verified | **Accepted** (the "state unknown" flag design; refinement: dim only what is actually unknown — liveness — while lines and history stay bright, they are database truth) | Fixes the restart flicker he watched: all green (yesterday's stored statuses) → offline (sweep) → question (grace) became a single transition: checking → live team *or* question. Generalizes D23's grace from the shift into the liveness layer itself; also removes the false-green moment on restarts with no shift |
| D40 | **Membership context by one SessionStart hook (`team-charter.md` §3).** Registration writes a `hooks.SessionStart` entry (matchers startup, resume, clear, compact, fork) into the `.claude/settings.local.json` it already writes; the entry runs `courtyard-claude-context --hub <url> --name <agent>` from courtyard's own venv, which asks the hub (`GET /api/agents/{name}/session-context`) for a short block naming the agent, the team, the channel `courtyard`, the kinds of message the hub sends and the delivery check, and falls back to the same text without the team's name when the hub is down. Ours by the command's name; replaced in place, removed on uninstall, other hooks untouched. The delivery check is reworded as an expected step of the operator's shift, without "tell no one, do nothing else", and worded per adapter (the MCP tool name only for Claude Code), asking for the one tool call and saying that the operator sees the result on the board; it carries a preamble of its own instead of the hub notice's "It is not a request", and the message footers name the tools per adapter the same way. The block names the agent, says the project's directory name does not matter, and tells the session to end its turn and wait when a courtyard message asks nothing more of it. The pi adapter gives the same block worded for pi (the extension instead of the channel, the skill instead of the MCP instructions): its extension stores it as a custom message at `session_start`, before attaching, leaves a session that already holds it alone, stores it again on `session_compact`, and falls back to the text install rendered into the file; `before_agent_start` is not used because pi fires it only for a typed prompt | **Accepted** (2026-09-11; extended to pi the same day) | Seen live in fresh directories (Claude Code 2.1.268, Sonnet 5): the host wraps channel events as untrusted external data, and a session with no user-side context refused the delivery check as prompt injection, then a peer's question; the agent whose operator had typed the ask used the tools at once. The hub cannot supply that context through the channel it is supposed to vouch for; the hook is the user's configuration and carries the user's authority, runs without a trust prompt, and costs one short block once per session. Hub-rendered rather than a text file in the workdir: the names stay current and the footprint stays at three files. A memory-file variant (`CLAUDE.md`) was rejected as touching the project's own file. On pi (gpt-5.6-luna, same day) the check was acked first, then the session spent 33 seconds and 15 tool calls working out which agent it was (its directory has another name), tried to answer "hub" and messaged the operator, as "You may mention it to your operator" invited; pi loads skills only on demand, so the etiquette skill alone did not reach it. With the block in place the name search stopped, but the session still spent 75 seconds and 32 more calls looking for work (messages to both peers, four refused sends to the operator, polling), under a check whose preamble said "It is not a request" and footers that named MCP tools its host does not have |
| D39 | **Registration's footprint in the workdir is told in full, kept out of git, and undone in one command (items 28 and 40)**: the install result carries a files notice (`.mcp.json` holds the token, chmod 600; `.claude/settings.local.json` is this machine's profile; a replaced file stays beside it as `*.courtyard-bak` and can hold a previous token; `start-with-courtyard.sh`, and pi's skill, carry no secret and may be committed), shown by the launch config panel and by `courtyard-invite`; when the workdir is a git checkout (`.git` present) the hub maintains the token-carrying names in its `.gitignore` under a marker line, created if missing, appended in place, never duplicated, and takes exactly those lines out again on disconnect (`POST /api/agents/{id}/disconnect`, the reverse of connect: files out, registration and charter untouched); a directory without `.git` is left alone (no `.gitignore` is invented for it); `courtyard-invite --unregister` is the full undo, files out then the agent removed from the hub and the charter, the WebUI's order, with `--disconnect` for the files alone; at hub start, after an upgrade or a URL change, the hub reconnects every registered agent's directory by itself and records the version and URL that wrote them (settings row `connected_by`) | **Accepted** | The one-line CLI warning named one file of four and offered no action, and the old `--remove` did half of what `--register` did. Reconnecting at start: an upgrade otherwise left every directory with the previous adapter until someone pressed a button per agent. Automatic rather than asked: the entries are the secret-carrying names and nothing else, and git is the one place a token must never land. The "a full undo burns the name" objection fell with D36 |
| D38 | **The installed hub: a macOS LaunchAgent, not a container (§9.4)**: `make install` (scripts/install.py, standard library only so it runs before the venv exists) writes two LaunchAgents, `com.courtyard.hub` (RunAtLoad, KeepAlive; runs scripts/hub-launch.sh, which sets a PATH launchd lacks, sources `.env`, waits for Docker, brings the compose postgres up and execs the venv's hub) and `com.courtyard.tray` (the menu bar app **Courtyard Admin**, rumps, an accessory with no Dock tile; a shell-script `~/Applications/Courtyard Admin.app` brings it back after Quit); `install.sh` (`curl | sh`) downloads the newest release zip into an empty directory and runs it; the hub learns it is supervised from `COURTYARD_SUPERVISED`, which only the plist sets, and only then accepts `POST /api/hub/restart` (a self-SIGTERM that KeepAlive turns into a restart; refused `not_supervised` otherwise, since a bare hub would simply stop); the hub ends the WebUI's event streams when a stop signal arrives (the event bus wakes them), so the process exits at once, and uvicorn's graceful shutdown stays bounded to a few seconds as a backstop (found live: an unbounded wait left the old process alive and launchd never restarted it; the bound alone then cancelled each stream with a logged traceback, and Ctrl+C on `make run` ended in a KeyboardInterrupt traceback); the install refuses to load the LaunchAgent while a hub that is not its own answers on the port; one machine, one courtyard database: the compose project is named `courtyard` (stable volume and container names across directories), postgres on host port 26432 so it never collides with a developer's own, and the migrator refuses to create the hub's schema in an unmigrated database that holds other tables; Adminer (`make db-ui`) and Swagger (`/api/docs`) are the two windows under the hood **The database's identity:** at startup, after migrations, the hub stamps the database with a random id in `settings` (`hub_identity`) or adopts the one there (one database per machine, shared by every checkout); every connection the pool hands out, and every health ping, compares it. A mismatch means another compose project or volume answers on the hub's postgres port: the hub logs it once, `/api/health` reports it, every request answers `503 foreign_database`, and only a restart adopts the new database. Seen live 2026-09-11: a second instance configured with its own compose project but the default ports lost its postgres, and the installed hub served a fresh dev database as its own for an hour | **Accepted** (the day-to-day mode; `make run` stays the development mode) | The hub was always meant to run on the host beside the operator's terminals (§8); a hub container (the earlier "live mode" idea) would put the spawner behind a container boundary for no gain. A remote hub is a postponed feature. The tray's dependency is a normal one on purpose: a plain `uv sync` removed an extra and left the tray restarting every 5 s |
| D37 | **Hub memory: the hub remembers collaboration, not craft (`hub-memory.md`, item 39)**: a closed thread becomes one case file, assembled from facts the hub already holds (participants with their domains, the ask, the resolution that got through, every verdict with its comment, the ordered messages), no model call; only closed threads count (expired and locked ones would skew later judgement), operator threads included; `courtyard_recall` is the pull path back into an agent's context, bounded (Admin `Recall returns`, default 5) and trimmed (`Recall trims to`, default 600 characters), ranked by Postgres full text with the ask and the participants' declared domains weighing most, filtered under `manual` discovery to the case files the agent appears in, rendered hub-side (D14), with a handle that fetches the full file; `GET /api/memory` is the same store for the Memory page and every other consumer; the hub never reads an agent's transcript or writes into its memory; no verdict is delivered outside the thread it belongs to; **notes** (slice 2, migration 0021): `courtyard_note` deposits a lesson scoped to one line unless team-wide, gated like a message (pending on a supervised line or team-wide; approve / return with comment / drop on the Memory page, return and drop reach the author as a hub notice), only accepted notes are memory, the operator's notes are accepted at once; **vectors** (slice 3, migration 0022): pgvector in the compose postgres (`pgvector/pgvector:pg18`), an untyped `embedding` column tagged with `embedding_model` so a model change is a re-embed rather than a new column, an `Encoder` interface (`none` default; any OpenAI-compatible endpoint on localhost, Ollama's `nomic-embed-text` documented; a fake bag-of-words encoder for tests; a non-local endpoint refused without `COURTYARD_EMBEDDINGS_ALLOW_REMOTE=1`), a background sweep that embeds records after the fact, and `hybrid` mode (full text and cosine similarity fused by reciprocal rank) as the default whenever an encoder exists; export and supersession are slice 4; visibility is keyed on the agent id, so a name revived under D36 inherits its previous life's case files and line notes (accepted, `hub-memory.md` section 8) | **Accepted** (slices 1 to 3 implemented; migrations 0020 to 0022) | The one thing no agent's own memory can hold is the exchange itself plus the operator's verdicts on it, and it evaporated at every End shift. Structure-not-prose keeps the hub model-free and every archive and foreign key valid; the options weighed, with votes, are in `hub-memory.md` section 9 |
| D36 | **A removed name can be registered again (§5.1, §5.7)**: registering a name whose agent was removed revives that agent's own row (same id) instead of refusing `name_taken`: new token, status `invited`, liveness fields cleared, `created_at` reset, and description, owns, anti-scope, workdir, model, colour and even the type taken from the new registration; a live name is still refused; a charter reload that names a removed agent revives it the same way (the files are the master); no migration, the `UNIQUE(name)` constraint stands | **Accepted** (an agent removed by mistake, or re-registered from a fresh charter, must get its name back) | Reverses the D20-era reading that names are permanent forever. Keeping the row is what keeps every foreign key (messages, threads, gate verdicts) and every archive valid; nothing of the old life survives but the id. Hard-deleting removed agents (the housekeeping action §10 mentions) was the alternative and is still possible later; `type` stays uneditable on a live agent because the installed files and the launch command differ per type |
| D35 | **Ghostty is a third fully driven terminal; the spawners share one base class (§8.1)**: Ghostty 1.3's scripting dictionary (surface configuration with `initial working directory` and `initial input`, `new window` returning a stable text id, `close window`) is enough for the shift; the agent's line is typed as `initial input` into the login shell (the Terminal.app model), and the shell reports its tty into a temp file because the dictionary exposes none; `OsascriptTerminal` holds the shared ref shape, liveness and close sequence, and every app counts its window BEFORE the kill (Ghostty always, Terminal.app under a close-on-exit profile, retire the window when the shell ends, so a count after the kill reads zero for a close that worked) and the count gates the kill: a gone window has no claim on its tty name, which macOS hands to the next window that opens, so signalling it would hit the operator's own shell; liveness likewise requires the window, not only a busy tty, and neither check ever launches a quit app; the tty probe uses `ps -t` (on macOS 26 `pgrep -t` / `pkill -t` match nothing, which had silently broken liveness and End shift's pre-close kill for every app); iTerm2 is addressed by bundle id (`application "iTerm2"` does not resolve, the app file is `iTerm.app`); the WebUI reads the built-in names from the hub | **Accepted** (Ghostty is the terminal in daily use; the base class replaced three near-identical classes) | Adding a terminal is now: subclass with the app's open script and three class attributes, one registry entry, one name in `BUILTIN_TERMINALS`, then the spawner procedure against the real app. The tty and iTerm2 fixes came out of running that procedure for the first time |
| D34 | **Threads: the quant of conversation (`threads.md`)** — a thread is one bounded exchange about one ask inside a line; every message belongs to exactly one; **serial in v1** (one open thread per line); the **sender declares boundaries, the hub never infers them** (open = a parameter on the send call; close = a dedicated bare tool call with no note field, the peer sees a fixed hub-rendered "thread closed by X" line); lifecycle open/closed/expired/locked; per-thread exchange budgets are the structural answer to item 29; operator threads unbudgeted (the D9 analog) and the operator closes via a control in the conversation pane; threads start at the migration, no backfill | **Accepted** (the full design discussion is `threads.md`) | The missing tier between the line and the message: closure becomes protocol instead of footer prose, the close instruction rides the envelope footer at the moment it applies, and the closed thread is item 39's natural digest unit |
| D33 | **The team charter: the team defined as files (item 41, `team-charter.md`)** — one directory per team, operator-chosen, typically the root of its own git repo, never inside an agent's workdir; **files are the source of truth**, the WebUI an editor and viewer that writes back (agent add/edit/remove included); the hub keeps a registry of teams with one current, and a current team is **required** — agent registration without one is refused (`no_team`), an empty courtyard asks for the team directory first (load it, or initialize an empty one), and choosing the first team on a hub that already holds agents adopts them into the charter (Admin Teams subpage; switching a running hub postponed); a load that registers an agent writes its launch config files into its workdir with the fresh token, and so does choosing an agent's workdir in the Teams view, so a charter loaded on a fresh hub needs no per-agent step before Start shift (agents already registered are not rewritten, a file that cannot be written is a load-report problem); `team-definition.yml` with a single team root maps agents to `agent-config-dir` subdirectories (card.yml plus description/owns/anti-scope prose files); workdirs stay per-machine, never in the shared charter; reload is explicit, the hub never watches the filesystem; hook-loaded context postponed; of the charter only one anti-scope line per peer reaches the envelope | **Accepted**, revised after live use (the full design discussion is `team-charter.md`; the driving use case: team setups shareable between engineers by publishing the charter repo) | Team design becomes reviewable in a pull request and portable between machines; enforced-versus-advisory is classified per item at implementation time |
| D32 | **The pi adapter is one native extension file (§7.3, item 36)** — our own extension speaking the unchanged §7.1 HTTP contract, chosen over reusing the MCP server through the third-party `pi-mcp-adapter` (tools would work, but pi ignores the Claude-Code-specific channel notification: the item-30 deaf agent by construction), over `pi-intercom` (direct pi-to-pi sockets, no hub in the path) and over pi's RPC mode (the hub would own sessions, against D14/D16). Delivery via `pi.sendMessage` custom message with `triggerTurn`/`followUp`; install writes the rendered template (token inline, 0600); shift launches `pi`, with `--model` when the agent declares a model; the wire round trip is tested by driving the identical file under Node against a live hub | **Accepted** (item 36, option A after the options research) | pi's extension API makes the hard half first-class: turn injection with polite queueing is documented behavior, not a drifting preview. The seam §7.1 promised is now proven by a second implementation with zero hub changes |
| D31 | **Install writes `start-with-courtyard.sh` (§7.2, item 35)** — a third install file: the human launch wrapper (`cd` to its directory, `exec` the launch command with the channel flag and `--model`, `"$@"` passed through, chmod 755). Regenerated on every install; carries no secret; a foreign file of that name is backed up once (never rotated away by re-installs); uninstall deletes ours by its marker comment, restores a backed-up foreign one, and never touches an unmarked stranger | **Accepted** ("we will tell user only 'run this script in the agent's dir'") | The channel flag is long and unmemorable, and forgetting it is exactly the deaf-session failure of items 30/33; D29's popup can now name a one-line remedy. Widens D14's footprint to three files, still configuration, not behaviour |
| D30 | **The delivery check: prove the model can hear, not just the adapter (§6.3, item 34)** — a synthetic hub-notice push (storage-less, riding the normal push payload) carries a nonce the model must return via the new `courtyard_ack` tool; ack → agent `delivery-verified` (point-in-time, shown with its time), timeout (`COURTYARD_VERIFY_TIMEOUT_SECONDS`, 60 s, swept) or refused push → `failed`, warned on the card. Triggered on any attach during an active shift and by the card's on-demand button; off-shift attaches stay quiet | **Accepted** (layer 2 of the item-33/34 split: ping on shift start + a small card button; the attach-during-shift trigger widens shift start to every risky session beginning with one code path) | The adapter ACKs pushes even when Claude Code drops the events, so transport pings prove nothing; a tool call is the only observable model reaction. Costs one small model turn per check — accepted. Every shift start now also exercises D14's open acceptance fact (a channel event starts a turn on an idle session) |
| D29 | **The channel-flag report: detect the deaf session deterministically (§6.3, §7.1, item 33)** — at startup the adapter walks its process ancestry (5 levels, `ps`), skips wrappers naming the adapter itself, and judges the claude launch command: flag present / absent / unknown; reported at attach, stored per channel (migration 0015), `absent` raises the board popup naming the agent and the restart remedy plus a red card foot. Only definite absence warns | **Accepted** (layer 1 of the split, from the item-30/31 post-mortem: "we did not detect that channels are not working and we did not show this to user"; popup included) | A bare `claude` restart attaches, heartbeats and ACKs pushes while Claude Code silently discards every channel event; the launch command is the only deterministic tell, and it would have turned that day's silent failure into a labeled one at attach |
| D28 | **Shift start verifies before it trusts (§8.1, §6.3)** — start calls the liveness layer's `begin_verification()`: stored `connected`/`stale` flip to `unknown` and the judging window reopens for one heartbeat interval + 5 s; the shift's grace runs until that verdict, so agents that prove themselves with a beat are skipped and everything unproven is spawned. The "instant start on an old hub" path is removed deliberately. Heartbeat default 15 s → 5 s (his call after the live check) so the countdown is 10 s | **Accepted** ("set heartbeat to off and wait for the next heartbeat before saying everything is green") | Fixes the skipped spawn he hit: agents dead since End shift kept their stored green for up to `gone_seconds` (600 s), a prompt re-start skipped them, opened nothing, and the stale-shift question fired minutes later. Extends D26 (verify before claiming) from hub startup to shift start — instant start was only ever as trustworthy as the stored claim |
| D27 | **The verdict comment moves inline and `reject` becomes `drop` (item 24)** — a held message carries its comment field between the body and the approve / return-to-sender / drop buttons (square-cornered: a form, not a chat); a drop's comment travels nowhere (kept on the board as the operator's record); the free-standing "note → both" leaves the UI (6a's use case dropped — a note question got answered into a terminal, and even a delivered answer lands on the operator line, not the pane it was asked in); delivered operator notes gain a reply-path footer (§7.5); status `rejected` → `dropped`, migration 0014 | **Accepted** (placement, box shape and single-destination rule as designed; `drop` reverses 3.2's "keep reject" — too close to "return to sender") | The line pane stops pretending to be a chat: the only thing the operator writes on a line is the verdict's comment, and questions belong in direct chats |
| D25 | **The stale shift asks a question (§8.1)** — the hub reports `stale` when the shift reads on, the liveness grace has passed, no launchable agent is connected, and none of the shift's windows has a live tty; the Courtyard page then shows a dialog with two answers: **End shift** (focused default — close it, nothing more; D24 expiry) and **Start new shift** (start on a stale shift = close old books, then fresh), plus Not now (amber "shift left open" tag remains, click to re-ask). **Amended same day during his check: Resume belongs to the living shift** — with part of the team down (1 of 2 healthy), the running pill offers `▶ Resume shift` (`POST /api/shift/resume`: start only the missing agents, dead spawn records retired, live windows never doubled, books and `started_at` untouched — §6.4 re-arm redelivers); the all-dead dialog is a binary, both answers close the books | **Accepted** (the dialog form and the resume option replaced the docs-only and pill-only proposals) | The operator's mental model wins: next morning there is no shift, whatever the record says — so the natural gestures (Start, or just answering a plain question) must work without understanding the bookkeeping. No hub reflex: stale is only ever *reported*; nothing closes or spawns until the operator answers. The windows-alive condition keeps a consent-stuck fresh start from reading as abandoned |
| D23 | **Shift + Team mode (§8.1)** — Team mode `On shift` (v1) \| `Always on` (future, disabled), changed only in Admin; one pill on the Courtyard page starts every registered agent not already up via its launch profile (terminal window, fire-and-forget, window ref recorded) and End shift closes exactly what it started; grace countdown before spawning only while the hub is younger than one heartbeat window; settings KV table (migration 0010); heartbeat default 30 s → 15 s | **Accepted** | Brings parked L1 (D8/D16) back into v1 as an operator gesture over the whole team — D14 intact. "Crew" rejected (the concept is already named Team); "Single User vs Service" rejected as the setting name (audience axis = §2 non-goals; lifecycle is what the setting controls). D22 holds the manual-links discovery mode |
| D21 | **Install writes the agent-side profile `.claude/settings.local.json` beside `.mcp.json`** — a permission rule pre-approving the courtyard MCP tools (feedback 7.2: without it every `courtyard_send` halts on a terminal prompt), the agent's declared model (`agents.model`, migration 0009; feedback 1 — also shown as `--model` in the suggested launch command), and a status line naming the agent (feedback 2), the status line only when none exists. Uninstall removes exactly what install added; the model stays | **Accepted** | Widens D14's footprint from one file to two, deliberately: still configuration, not behaviour — no hooks. The file is per-machine (git-excluded by Claude Code when it writes it itself) and carries no secret. Verified against the Claude Code docs: `mcp__courtyard` allows all the server's tools; `model` accepts aliases like `sonnet`; the one-time trust dialog for a project's `.mcp.json` servers cannot be pre-approved and remains |

## 14. Risks and required spikes

1. **`claude/channel` MCP capability drift** — ~~spike required~~ **RESOLVED by spike 6a
   (see D-spike)**: the capability graduated into the official "channels"
...
4. **macOS-only L1** — live again since the Shift (§8.1, D23) brought the spawn back into
   v1: Start shift drives Terminal.app, iTerm2 or Ghostty via `osascript`, so shift-launching is
...
- Prior art (same author): Postgres-backed queue / "one transaction domain" design in
